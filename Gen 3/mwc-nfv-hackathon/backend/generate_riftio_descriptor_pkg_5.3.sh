...
############################################################################
# Copyright 2016-2018 RIFT.io Inc                                               #
#                                                                          #
...
# Example: generate_descriptor_pkg.sh -c -a --nsd --vcpu <vcpu_cnt> --memory <memory_mb> --storage <storage_gb> --image <image-name> <base-dir> <vnf-name>
...
Usage:
     $SCRIPTNAME [-t <type>] [-N] [-c] [base-directory] <package-name>
        ...
        Options specifc for create descriptor:
            ...
            --version : Version for descriptor. Default 1.0
          ...
          VDU Parameters:
            ...
            --interfaces : Number of external interfaces excluding management interface.
	                   Default 0.
    ...
    cat >>$desc_file <<EOF
            -   vnfd:name: eth0
                vnfd:virtual-interface:
                    vnfd:type: VIRTIO
                vnfd:external-connection-point-ref: ${name}/cp0
                vnfd:type: "EXTERNAL"
                vnfd:position: 1
EOF
    ...
    for i in `seq 1 ${INTERFACES}`; do
        cat >>$desc_file <<EOF
            -   vnfd:name: eth${i}
                vnfd:virtual-interface:
                    vnfd:type: ${INTF_TYPE}
                vnfd:external-connection-point-ref: ${name}/cp${i}
                vnfd:type: "EXTERNAL"
                vnfd:position: $((i+1))
EOF
    done
    ...
    cat >>$desc_file <<EOF
            -   nsd:id: ${name}_vld0
                ...
                -   nsd:member-vnf-index-ref: 1
                    nsd:vnfd-id-ref: ${vnfd}
                    nsd:vnfd-connection-point-ref: ${vnfd}/cp0
EOF
...
function generate_package(){
    ...
    PRODUCT_NAME="${name}"
    initialize_manifest_file $name".yaml"

    if [ $ARCHIVE == true ]; then
        # Create archive of the package
        cd $dest_dir
        if [ $VERBOSE == true ]; then
            tar zcvf ${name}.tar.gz ${name} ${RM}
            echo "Created package ${name}.tar.gz in $dest_dir"
        else
            tar zcvf ${name}.tar.gz ${name} ${RM} >/dev/null 2>&1
        fi
...
OPTS=`getopt -o vhnt:d:caN --long verbose,dry-run,help,package-type:,destination-dir,create-folder,no-remove-files,archive,nsd,version:,vendor:,interface-type:,vcpu:,memory:,storage:,image:,cloud-init-file:,cloud-init:,interfaces:,product-name:,package-version:,debug -n $SCRIPTNAME -- "$@"`
...
while true; do
    case "$1" in
        ...
        --product-name ) PRODUCT_NAME=$2; shift; shift;;
        --package-version ) PACKAGE_VERSION=$2; shift; shift;;
...
function initialize_manifest_file() {
    dirs=$( find * -maxdepth 0 -type d )
    name=${PRODUCT_NAME}
    mf_file="${1%".yaml"}.mf"
    datetime="$(date -u '+%Y-%m-%dT%H:%M:%S')"
    echo "Product-Name: ${name}" > $mf_file
    echo "Provider-ID: ${VENDOR}" >> $mf_file
    echo "Package-Version: ${PACKAGE_VERSION}" >> $mf_file
    echo "Release-Date-Time: ${datetime}" >> $mf_file
    echo "Package-State: new" >> $mf_file

    hash="$(sha256sum $1)"
    echo "" >> $mf_file
    echo "Source: ${1}" >> $mf_file
    echo "Algorithm: SHA256" >> $mf_file
    echo "Hash: "${hash%" $1"}"" >> $mf_file
		...
		if [ $d == "images" ]; then
                    find images -type f  2>/dev/null|
                        while read file; do
                            hash="$(md5sum $file)"
                            echo "" >> $mf_file
                            echo "Source: ${file}" >> $mf_file
                            echo "Algorithm: MD5" >> $mf_file
                            echo "Hash: "${hash%" $file"}"" >> $mf_file
                        done
		else
...
if [ $CREATE == false ]; then
    ...
    initialize_manifest_file $descriptor $mf_file $desc_sub_dir
